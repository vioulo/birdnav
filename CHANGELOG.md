...
### Added
- 后台侧边栏导航添加 lucide-react 图标，提升菜单项可辨识度。
- 新增后台页面骨架屏（Skeleton Loading），站点管理、分类管理、操作日志、基础配置页面在数据加载时展示结构化占位动画，页头保持可见。
- 新建 / 编辑 / 批量导入站点时自动抓取目标网站的 meta description 填充简介，手动填写优先。
- 前台所有外部站点链接自动追加 `utm_source` 参数，值动态取自当前域名，便于目标站点追踪流量来源。
- 新增 `SubmitButton` 客户端组件，表单提交时自动显示 loading spinner 并禁用按钮。

### Changed
- 后台弹窗（新建 / 编辑 / 批量导入）添加玻璃拟态效果，优化表单与模态框样式。
- 后台管理面板整体 CSS 优化，改进侧边栏、内容区、分页、表格的视觉层次。
- 站点详情页描述区域样式调整。
- 更新 favicon，体积大幅缩小（25KB → 1.6KB）。
